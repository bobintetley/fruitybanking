...
def getTransactions(accountid, num):
    ...
    # If num is 0, we want to view all transactions so make
    # the cutoffdate 1990 or something
    if num == -1:
        cutoffdate = toUnixDate(datetime.date(1990, 1, 1))
    else:
        # Work out our cutoff point from the number of rows selected
        cutoffdate = getCutOffDateForRows(accountid, num)
    
        # If cutoffdate is zero, then there were no rows
        if cutoffdate == 0:
             return l
    
    # Retrieve all the relevant transaction records
    d = db.runQuery("SELECT ID, Date, Description, Reconciled, Amount, SourceAccountID, DestinationAccountID FROM trx WHERE Deleted = 0 AND Date >= %s AND (SourceAccountID=%s OR DestinationAccountID=%s) ORDER BY Date" % (cutoffdate, accountid, accountid))

    # Start the cumulative balance upto (but not including) the cut off date
    balance = accounts.getAccountBalanceToDate(accountid, cutoffdate)
...
def getCutOffDateForRows(accountid, num):
    """
    ...
    """
    d = db.runQuery("SELECT Date FROM trx WHERE (SourceAccountID=%s OR DestinationAccountID=%s) AND Deleted = 0 ORDER BY Date DESC LIMIT %s" % (accountid, accountid, num))
...
def getNumberOfTrxToShow():
    ...
    """
    return 50
    #if "NumberOfTrx" not in cpg.request.sessionMap:
    #    return 20
    #else:
    #    return int(cpg.request.sessionMap["NumberOfTrx"])
    
def setNumberOfTrxToShow(number):
    """ 
        Sets the number of transactions to show in 
    the user's session. 
    """
    #cpg.request.sessionMap["NumberOfTrx"] = number
    pass
